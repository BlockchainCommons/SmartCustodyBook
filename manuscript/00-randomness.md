...
[^1]: A "bit of randomness" is the standard way to measure the strength of a random number. It's a single digit that can be set to either a "1" or a "0". 64 bits of randomness is therefore 64 digits that can each be set to either a "1" or a "0". That may not sound like a lot, but every bit you add doubles the possibilities: there are thus two options for a one-bit number ("0", "1"), four for a two-bit number ("00", "01", "10", "11"), eight for a three-bit number, etc. It quickly adds up.
